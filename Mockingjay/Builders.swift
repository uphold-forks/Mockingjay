...
public func failure(error:NSError) -> (_ request:URLRequest) -> Response {
  return { _ in return .failure(error) }
}

public func http(status:Int = 200, headers:[String:String]? = nil, data:Data?=nil) -> (_ request:URLRequest) -> Response {
...
public func json(body:Any, status:Int = 200, headers:[String:String]? = nil) -> (_ request:URLRequest) -> Response {
  return { (request:URLRequest) in
    do {
      let data = try JSONSerialization.data(withJSONObject: body, options: JSONSerialization.WritingOptions())
      return jsonData(data: data, status: status, headers: headers)(request)
    } catch {
...
public func jsonData(data: Data, status: Int = 200, headers: [String:String]? = nil) -> (_ request:URLRequest) -> Response {
  return { (request:URLRequest) in
    ...
    return http(status: status, headers: headers, data: data)(request)
  }
}
